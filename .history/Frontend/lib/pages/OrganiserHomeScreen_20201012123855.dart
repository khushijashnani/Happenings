...
class _OrganiserHomeScreenState extends State<OrganiserHomeScreen> {
  ...
  Widget buildBarGraph() {
    return Padding(
      ...
      child: Container(
        height: 300,
        width: series.length <= 6 ? screenWidth : screenWidth + screenWidth * ((series.length - 6) * 0.1),
        child: ShaderMask(
  ...
  Widget drawGroupGraph() {
    return Container(
        height: 300,
        width: series_group.length <= 5 ? screenWidth : screenWidth + screenWidth * ((series_group.length - 5) * 0.1),
        child: charts.BarChart (
            series_group,
            animate: true,
            animationDuration: Duration(milliseconds: 3000),
